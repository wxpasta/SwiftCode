...
class PlayingCardView: UIView {
    ...
    private var cornerString: NSAttributedString{
        let string = rankString+"\n"+suit
        return centeredAttributedString(string, fontSize: cornerFontSize)
    }
    ...
    private func drawPips()  {
        ...
        func creatPipString(thatFits pipRect: CGRect) -> NSAttributedString{
            ...
            let verticalPipRowSpacing = pipRect.height / maxVerticalPipCount
            let attemptedPipString = centeredAttributedString(suit, fontSize: verticalPipRowSpacing)
            let probablyOKayPipStringFontSize = verticalPipRowSpacing / (attemptedPipString.size().height / verticalPipRowSpacing)

            let probablyOKayPipString = centeredAttributedString(suit, fontSize: probablyOKayPipStringFontSize)
            if probablyOKayPipString.size().width > pipRect.height / maxHorizontalPipCount {
                return centeredAttributedString(suit, fontSize: probablyOKayPipStringFontSize / probablyOKayPipString.size().width / (pipRect.width / maxHorizontalPipCount))
            }else{
        ...
        if pipsPerRowForRank.indices.contains(rank) {
            ...
            let pipsRowSpaing = pipRect.height / CGFloat(pipsPerRow.count)
            pipRect.size.height = pipString.size().height
            pipRect.origin.y += (pipsRowSpaing - pipRect.height) / 2
            for pipCount in pipsPerRow{
                ...
                pipRect.origin.y += pipsRowSpaing
            }
